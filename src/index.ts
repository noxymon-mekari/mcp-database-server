...
async function initFromConfig(configPath: string) {
  ...
  for (const f of failures) {
    logger.warn(`Failed to connect to database "${f.name}": ${f.error}`);
  }

  logger.info(`${successes.length}/${config.databases.length} database(s) connected successfully`);
}

/**
 * Initialize a single database from CLI arguments (legacy mode)
 */
async function initFromCliArgs() {
  let dbType = 'sqlite';
  ...
  await initDatabase(connectionInfo, dbType, dbName);
  logger.info(`Connected to ${dbType} database as "${dbName}"`);
}
